...
bool flagHab = true;
int pasos = 0;
bool flagPasos = false;
int pasosBluetooth = 0;
//------------------------------------
...
void loop() {
  ...
  //  float pastZ = acel.getZ();
    xin = acel.aTotal();
    yout = 0.2283*yant + 38.587*xin - 38.587*xant;
    ...
    if (youtF >= 350 && flagHab == true){
      pasos++;
      flagHab = false;
      flagPasos=true;
    }
    if (youtF < 0 && flagHab==false){
      flagHab = true;
    }
    if((pasos%2 == 1) && flagPasos==true){
      pasosBluetooth++;
      flagPasos=false;
      char str[5]="";
  ...
  if (botonFun(pinButton , &statPrevBut, &millisTime)) {
    for (int j = 1; j <= (pasosBluetooth+1); j++) {
      digitalWrite(led, LOW);
      delay(200);
      digitalWrite(led, HIGH);
      delay(200);
