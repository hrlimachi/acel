...
void loop()
{
  ...
  String cad = String(event.acceleration.x) + " " + String(event.acceleration.y) + " " + String(event.acceleration.z) + " " + String(total)+"\n";
  Serial.println(cad);
  //copyTotal = total;
  copyCad = cad;
...
void readSensor(void *parameter)
{
  ...
  {
    if (readySend == true) {
      //     char texto = (copyTotal);
      const char* string1 = copyCad.c_str();
      //dtostrf(copyTotal, 8, 2, str);
      appendFile(SD, "/data.txt",string1);
      readySend = false;
    }
    else
    {
