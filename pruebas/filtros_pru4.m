...
for i=1:cantidadArchivos
    ...
    archivoImportar = fullfile(rutaArchivo, strcat(filename,'.mat'));    % /8_car/data5.mat
    load(archivoImportar);
    
    % Especifica el nombre del directorio que deseas crear
    nombreDirectorioFiltro = fullfile(nombreDirectorio,filename);        % /8_car/data5/
    ...
    for frecCorte=10:10
        ...
        for j=1:length(total)
            ...
            xin2=yout(j);
            youtF(j)=(xin2+xN1+xN2)/3;%3.546*youtN1 - 4.873*youtN2 + 3.071*youtN3 - 0.749*youtN4 + 0.01226*xN1 - 0.01451*xN2;
