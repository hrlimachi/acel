...
for i=1:cantidadArchivos
    ...
    for frecCorte=1:1
        ...
        for j=1:length(total)
            xin=z(j);%total(j);
            %yout(j)=-den(2)*yantN1-den(3)*yantN2+num(1)*xin+num(2)*xantN1+num(3)*xantN2;
           % yout(j)=-den(2)*yant + num(1)*xin + num(2)*xant;%0.5219*yant + 0.2391*xin + 0.2391*xant;%0.6828*yant + 0.1586*xin + 0.1586*xant;
           % xin2=yout(j);
            youtF(j)=(xin+xN1+xN2)/3;%3.546*youtN1 - 4.873*youtN2 + 3.071*youtN3 - 0.749*youtN4 + 0.01226*xN1 - 0.01451*xN2;
         yout(j)=-den(2)*yantN1-den(3)*yantN2+num(1)*youtF(j)+num(2)*xantN1+num(3)*xantN2;
              yantN2=yantN1;
            yantN1=yout(j);
            xantN2=xantN1;
            xantN1=youtF(j);
